...
import sys
sys.path.insert(0, '/home/tdhoffman/Documents/spycause')
...
# params = list(itertools.product(treat, z_conf, y_conf, interf, ucar_str, vcar_str,
                                # balance, w_I.keys(), w_I.keys(), w_C.keys(), w_C.keys()))
params = list(itertools.product(interf, ucar_str, vcar_str, w_I.keys(), w_I.keys(), w_C.keys(), w_C.keys()))

## Loop
ncores = 32
ntasks = len(params)
chunksize = ntasks//(ncores - 1)
outfile = "../outputs/car_confounds_spstruct.json"


def chunk_sim_run(params):
    interf, ucar_str, vcar_str, w_I_data_form, w_I_model_form, w_C_data_form, w_C_model_form = params

    # CAR model MUST model spconfounding somehow
    if w_C_model_form == "none":
        return

    print("w_I_data_form:", w_I_data_form)
    print("w_I_model_form:", w_I_model_form)
    print("w_C_data_form:", w_C_data_form)
    print("w_C_model_form:", w_C_model_form)
    ...
    sim = spy.CARSimulator(Nlat, D, sp_confound=w_C[w_C_data_form], interference=intval)
